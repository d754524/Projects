...
import edu.uci.ics.jung.algorithms.layout.CircleLayout; //directed graphs
...
class SimGUI {
	...
	public void makeGraphPanel() {
		...
		Layout<ThreeTenNode, ThreeTenEdge> layout = new CircleLayout<ThreeTenNode, ThreeTenEdge>(graph);
		...
		context.setVertexShapeTransformer(
			new Transformer<ThreeTenNode,Shape>(){
				public Shape transform(ThreeTenNode v) {
					int s = 60;
					return new Ellipse2D.Double(-s/2.0, -s/4.0, s, s/2.0);
				}
			}
		);
